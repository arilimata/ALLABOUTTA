...
class DJIDroneLogParser(interface.FileObjectParser):
    ...
    def parse_gps(self, value):
        ...
        for x in records:
            if x[3] == 2096:
                payload = struct.unpack("<IIiiifff", get_payload(x[0], x[1], x[2])[:32])
                date = payload[0]
                time = payload[1]
                ...
                gps_records.append([year, month, day, hour, minute, second, longitude, latitude, height, x_offset, y_offset, z_offset])
    ...
    def ParseFileObject(self, parser_mediator, file_object):
        ...
        for record in records:            
            if all(v == 0 for v in record[:6]):
                ...
                # timestamp = calendar.timegm((record[0], record[1], record[2], record[3], record[4], record[5]) + (0, 0, 0)) + (7*3600)
                time_structure = (f"{record[0]:04d}-{record[1]:02d}-{record[2]:02d} {record[3]:02d}:{record[4]:02d}:{record[5]:02d}")
