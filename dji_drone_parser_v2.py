...
class DATFile:
    ...
    def search(files, root_path):
        ...
        for file in files:
            ...
            for key, value in DATFile.extract_dat(dat_path).items():
                if key.endswith(".DAT"):
...
""" Testing Code on DRDP [HAPUS SAAT FIKSASI]"""
# def TesDRDP():
#     root_path = r"./dats"
...
def TesDRDP():
    root_path = r"./dats"
    files = os.listdir(root_path)
    DATFile.search(files, root_path)
    
    
    # # Define the regex pattern for the desired file format
    # pattern = re.compile(r'^\d{2}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}_FLY\d{3}\.DAT$')
    # for file in files:
    #     # Skip files that do not match the regex pattern
    #     if not pattern.match(file):
