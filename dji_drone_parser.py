...
import os
import traceback
import csv # Hapus pada final
from datetime import datetime

# from plaso.parsers import interface
# from plaso.parsers import manager

class DJIDroneLogEventData():
...
class DATFile:
    def __init__(self, data):
        ...
        # print(acconfig.get(self.data[o], ("Unknown", 4500000.0)))
        # print("FWData:", self.data[21:33].strip())
        ...
        while True:
            ...
            try:
                if self.data[cur_pos] != 0x55:
                    cur_pos += 1
                    raise Exception(f"Data Corrupt at position:{cur_pos - 1}")
                ...
                """ PERCOBAAN TANPA OBJECT RECORD """
                # Gapake object, langsung berupa list array
                record_list.append([cur_pos,
                                    record_len, 
    ...
    def parse_gps(self):
        ...
        # for v in [x for x in self.parse_records() if x.type == 2096]: # Looping asli
            # date, time, longitude, latitude = struct.unpack("<IIii", get_payload(v)[:16])
        ...
        for v in [x for x in self.parse_records() if x[3] == 2096]: # Pakai array dirubah checking pada object list ke-4
            # print(v)
            # os.system("pause")
...
class TesDRDP():
    ...
    for file in files:
        if not file.endswith('.DAT'):
            continue
        csvfile = open(os.path.join(root_path, file + '.csv'), 'w', newline = '')
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['Time', 'Longitude', 'Latitude'])
        
        dat_path = os.path.join(root_path, file)
        print('File : ', dat_path)
        for key, value in extract_dat(dat_path).items():
            if key.endswith(".DAT"):
                for record in DATFile(value).parse_gps():
...
"""Todo List
1. Cek Parser di Plaso
2. Cari unique pada artefak agar bisa dijadikan key parsing 
3. Integrasikan pada Plaso
"""