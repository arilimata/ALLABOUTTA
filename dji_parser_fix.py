...
class DJIDroneLogParser(interface.FileObjectParser):
    ...
    def ParseRecords(self):
        ...
        while True:
            ...
            # cur_pos_output.append(record_len)  #Debug
            
            if record_len < 10 or cur_pos + record_len > len(self.data):
                cur_pos += 2
                # raise Exception("Length Corrupt!")
                cur_pos_output.append(cur_pos)
                cur_pos=self.FindNext(cur_pos)
                if cur_pos == -1:
                    break
            ...
            crc = DJIDroneLogParser.CheckSum(self.data[cur_pos:cur_pos + record_len - 2])
            ...
            record_list.append([cur_pos, # Current Position
                                ...
                                struct.unpack("<I", self.data[cur_pos + 6:cur_pos + 10])[0], # Ticket Number
                                (self.data[cur_pos + 5] << 8) + self.data[cur_pos + 4]]) # Type
            ...
            #     # cur_pos = self.FindNext(cur_pos)
            #     # cur_pos_output.append(self.FindNext(cur_pos))
            #     cur_pos = self.FindNext(cur_pos)
            #     if cur_pos == -1:
            #         break
        # return cur_pos_output
        return record_list

    def parse_gps(self, value):
        gps_records = []
        self.data = value
        ...
        def get_payload(cur_pos, record_len, ticket_no):
        ...
        for x in records:
            if x[3] == 2096:
                payload = struct.unpack("<IIii", get_payload(x[0], x[1], x[2])[:16])
                date = payload [0]
                time = payload [1]
                longitude_raw = payload[2]
                latitude_raw = payload[3]
                
                # date, time, longitude, latitude = struct.unpack("<IIii", get_payload(x[0], x[1], x[2])[:16])
                year = int(date / 10000)
                ...
                timestamp = str(datetime(year, month, day, hour, minute, second))
                
                # timestamp = datetime_elements.Filetime.CopyFromDateTimeString(timestamp)
                longitude = longitude_raw / (10 ** 7)
                latitude = latitude_raw / (10 ** 7)
                
                # collect = [timestamp, longitude, latitude]
                
                # gps_records.append([timestamp, longitude, latitude])
                gps_records.append([year, month, day, hour, minute, second, longitude, latitude])
        # for v in [x for x in self.ParseRecords() if x[3] == 2096]:  # Filter by record type
        #     date, time, longitude, latitude = struct.unpack("<IIii", get_payload(v[0], v[1], v[2])[:16])
        ...
        #     # """ Debugging """
        #     # event_data.parse_gps = self.data
            
        #     gps_records.append([timestamp, longitude, latitude])

        
        # output.append(self.ParseRecords()) # Debugging
        return gps_records
        # return records # Debugging
    ...
    def ParseFileObject(self, parser_mediator, file_object):
        # with file_object.GetFileObject as file_obj:
        #     data = file_obj.read()
        # self.data = str(file_object.read())
        path = file_object.read()
        # self.data = str(type(path))
        # f = open(path, 'rb')
        # value = f.read()
        # f.close()
        ...
        for record in records:
            ...
            # formatted_timestamp += formatted_offset
                        
            
            posix_time = time.mktime(datetime(record[0], record[1], record[2], record[3], record[4], record[5]).timetuple())
            
            event_data = DJIDroneLogEventData()
            
            event_data.timestamp = dfdatetime_posix_time.PosixTime(timestamp=posix_time)
            # event_data.timestamp = formatted_timestamp
        ...
        # event_data.timestamp = records
        # event_data.latitude = record[1]
        # event_data.longitude = record[2]
        # event_data.output = str("'" + str(path[242:252]))
        # event_data.output = str(self.record_start_pos)
        # event_data.output = str(records)
        # parser_mediator.ProduceEventData(event_data)
